...
1. **GitHubリポジトリとの接続とプッシュ**
   - GitHubで空のリポジトリ（名前は `sandbox` など）を作成し、ローカルリポジトリと紐付けてプッシュするよ！
   - 具体的な手順やコマンドは [git_setup_memo.md](file:///g:/work/sandbox/git_setup_memo.md) を参考にしてね！
2. **Java / Spring Boot 開発環境の準備とAPI設計**
   - プッシュが終わったら、いよいよSpring Bootの実装に向けて、Swagger (OpenAPI) を使ったAPI設計に進むよ！
